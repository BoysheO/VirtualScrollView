 ...
 * refactor by boysheo
 * 使用教程：
 * 按传统Scroll View挂载后，在Inspector面板中对ViewPort、Content、Prefab赋值；Prefab是列表中的条目GameObject
 * 在代码里对VirtualScrollView的numItems赋值一个较大的数，就可以看到效果了，此时每个条目的gameObject.name为index
 * 可以监听onItemRender事件里面进行一些操作
 */
...
namespace UnityEngine.UI
{
    ...
    public class VirtualScrollView : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutGroup
    {
        #region 定义结构
        ...
        /// <summary>
        /// 指示此组件是否被操作中（滚动、拖动）<br />
        /// *只对拖动操作做了适配。拖动滚动条的话，这个状态可能不正确
        /// </summary>
        public bool IsControlling => m_Dragging;
        ...
        [Obsolete("施工中的功能")][NonSerialized]
        public Vector2 StarPosOffset = new Vector2(0, 0);
        ...
        private float GetCellSize(MotionType axis)
        {
            return m_ItemSize[(int)axis] + m_Space[(int)axis];
        }
                    ...
                    if (!tmpContains.ContainsKey(virIdx))
                    {
                        ...
                        if (m_MotionType == MotionType.Vertical)
                        {
                            Vector2 offset = new Vector2((j + 0.5f)*itemSize.x + j*m_Space.x,
                                                -(line + 0.5f)*itemSize.y - line*m_Space.y)+StarPosOffset;
                            if (!m_Loop)
                        ...
                        else
                        {
                            Vector2 offset = new Vector2((line + 0.5f)*itemSize.x + line*m_Space.x,
                                                -(j + 0.5f)*itemSize.y - j*m_Space.y)+StarPosOffset;
                            if (!m_Loop)
                                item.position = new Vector2(margin.left, margin.top) + offset;
                            else
            ...
            if (m_Loop)
            {
                if (m_MotionType == MotionType.Horizontal)
                {
                    int index = target != int.MaxValue
                        ...
                        : GetLoopIndex(-GetCellSize(MotionType.Horizontal) * 0.5f);
                ...
                else
                {
                    int index = target != int.MaxValue
                        ? GetClosestLoopIndex(target)
                        : GetLoopIndex(GetCellSize(MotionType.Vertical) * 0.5f);
        ...
        private Vector2 CalculateClosestPosition(int target, float axis, SeekOrigin seekOrigin)
        {
            var v1 = CalculateClosestPosition(target);
            switch (motionType)
            {
                case MotionType.Horizontal:
                    v1.x += viewport.rect.width * axis;
                    break;
                case MotionType.Vertical:
                    v1.y += viewport.rect.height * axis;
                    break;
                default:
            ...
            switch (seekOrigin)
            {
                ...
                {
                    var cellSize = m_ItemSize[(int)motionType];
                    v1[(int)motionType] -= cellSize;
                    break;
                }
                case SeekOrigin.Current:
                {
                    var cellSize = m_ItemSize[(int)motionType];
                    v1[(int)motionType] -= cellSize * 0.5f;
                    break;
                }
                default:
        ...
        /// 
        /// </summary>
        /// <param name="index">设置为int.Max时，会自动对齐</param>
        /// <param name="duration"></param>
        /// <param name="axis"></param>
        /// <param name="seekOrigin"></param>
        public void ScrollTo(int index, float duration, float axis,SeekOrigin seekOrigin)
        {
            velocity = Vector2.zero;
            autoScrollState.Enable = true;
            autoScrollState.Duration = duration;
            autoScrollState.StartTime = Time.unscaledTime;
            autoScrollState.EndScrollPosition = CalculateClosestPosition(index, axis,seekOrigin);
        }
        ...
        private IEnumerator RebuildList()
        {
            ...
            m_MaxInit = Mathf.CeilToInt(GetViewSize() / GetCellSize(m_MotionType)) + 1;
        ...
        {
            int index = 0;
            float anchoredPositionXorY = m_MotionType == MotionType.Horizontal
                ? m_Content.anchoredPosition.x
                : m_Content.anchoredPosition.y;
            anchoredPositionXorY = Mathf.Round(anchoredPositionXorY) + offset;
            if (m_Loop)
            {
                if (m_MotionType == MotionType.Horizontal)
                {
                    index = Mathf.FloorToInt(-anchoredPositionXorY / GetCellSize(MotionType.Horizontal));
                }
                else
                {
                    index = Mathf.FloorToInt(anchoredPositionXorY / GetCellSize(MotionType.Vertical));
                }
            }
            else
            {
                if (m_MotionType == MotionType.Horizontal)
                {
                    index = anchoredPositionXorY >= 0 ? 0 : Mathf.FloorToInt(Mathf.Abs(anchoredPositionXorY + m_Margin.left) / GetCellSize(MotionType.Horizontal));
                }
                else
                {
                    index = anchoredPositionXorY < 0 ? 0 : Mathf.FloorToInt(Mathf.Abs(anchoredPositionXorY - m_Margin.top) / GetCellSize(MotionType.Vertical));
                }
            }
